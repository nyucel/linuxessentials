...
Neyse ki, günümüzde birçok dağıtım renkli kullanımı bünyesinde barındırıyor. Tablo 6.1 en yaygın atamaları göstermektedir.

Siyah-beyaz ekranlarda, ki hala günümüzde var, \emph{-F} veya \emph{-p} parametreleri önerilir. Bunlar dosya türleri ile ilişkili dosya isimlerine özel karakter eklerler. Bu karakterlerin bazıları Tablo 6.1'de verilmiştir.
...
İlk durumda, bütün gizli olmayan dosyalar listelendi; ikinci durumda ise ek bilgiler eklendi.
	...
	Uzun formatın farklı parçalarının anlamları vardır: İlk karakter dosya türünü verir (Bölüm ~\ref{chap:bolum10}'a bakınız); düz dosyalar "-", dizinler "d" şeklinde vb.(Tablo 6.1'deki "karakter türleri").
	
	Sonraki dokuz karakter erişim izinlerini gösterir. Sonraki dosyanın kaynak sayacını, dosyanın sahibini(\emph{joe}) ve dosyanın grubunu(\emph{users}) gösterir. Bayt olarak dosyanın boyutundan sonra, dosya içeriğinin son değiştirilme bilgisini tarih ve saat olarak görebilirsiniz. En sağda dosyanın adı bulunur.
	
	Kullandığınız dile bağlı olarak özellikle tarih ve saat sütunları örneklerimizden (asgari dil ortamı "C" kullanarak oluşturduğumuz) tamamen farklı görünebilir. Bu genellikle etkileşimli kullanımda sorun çıkarmaz, fakat eğer kabuk betiğinizin bir bölümünde " \emph{ls -l}" komutunun çıktısını almaya çalışıyorsanız büyük bir sıkıntı ortaya çıkabilir. (\emph{Gelişmiş Linux} kılavuzunu çalışmadan önce, kabuk betiklerinde ön tanımlanmış dil çevreselini kullanmanızı öneririz.)
	
	Eğer dizin için ekstra bilgi görmek isterseniz(\emph{/tmp} dizini gibi), "\emph{ls -l /tmp}"'nin size gerçekten bir yardımı dokunmaz, çünkü \emph{ls} komutu \emph{/tmp} içerisindeki tüm dosyaların verisini listeleyecektir. \emph{-d} seçeneğini kullanarak \emph{/tmp}'nin kendisinin bilgisini elde edebilirsiniz.
	
\emph{ls} burada bahsedilenlerden çok daha fazlasına destek verir; önemli olanlarından bir kaçı Tablo 6.2'de gösterilmişti.

\emph{Linux Essentials} ve LPI-101 gibi LPI sınavlarında, kimse sizden \emph{ls}'nin tüm 57 parametresini bilmenizi beklemez. Ancak en önemli yarım düzine kadarını, yaklaşık Tablo 6.2. içeriğindeki kadarını, öğrenmek isteyebilirsiniz.
\paragraph{Alıştırmalar}{
\begin{itemize}
