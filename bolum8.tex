...
\paragraph{cat}{Dosyaları birleştirir (diğer şeylerin yanında)}
\paragraph{cut}{Girdisindeki sütun ya da alanları ayırır}
\paragraph{head}{Bir dosyanın başlangıcını gösterir}
\paragraph{paste}{Farklı girdi dosyalarındaki satırları birleştirir}
\paragraph{reset}{Bir terminal karakterini mantıklı bir değere sıfırlar}
\paragraph{sort}{Girdisini satırlara göre sıralar}
\paragraph{tail}{Bir dosyanın sonunu gösterir}
\paragraph{uniq}{Bir dosyadaki tekrarlayan satırları vurgular.}
}
